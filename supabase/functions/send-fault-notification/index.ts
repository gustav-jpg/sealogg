...
serve(async (req) => {
  ...
  try {
    ...
    const payload: FaultPayload = await req.json();
    console.log("Received fault notification payload:", payload);
    ...
    // Find admins who want fault notifications
    const { data: preferences, error: prefError } = await supabase
      .from("notification_preferences")
      .select("user_id")
      .eq("organization_id", vessel.organization_id)
      .eq("email_new_faults", true);
    ...
    if (!preferences || preferences.length === 0) {
      console.log("No users opted in for fault notifications");
    ...
    const portalUrl = "https://sealogg.lovable.app/portal/faults";
      ...
      <body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f4f4f5;">
        <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 100%); padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
            <h1 style="color: #ffffff; margin: 0; font-size: 24px;">🔧 Nytt felärende</h1>
          </div>
          ...
          <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
              ...
              <tr>
                <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0;">
                  <span style="color: #64748b; font-size: 14px;">Prioritet</span>
                </td>
                <td style="padding: 10px 0; border-bottom: 1px solid #e2e8f0; text-align: right;">
            ...
            <div style="text-align: center; margin-top: 25px;">
              <a href="${portalUrl}" style="display: inline-block; background: linear-gradient(135deg, #0ea5e9 0%, #0284c7 100%); color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">
                Visa i Sealogg
              </a>
            </div>
          </div>
          ...
          <div style="text-align: center; padding: 20px;">
            <p style="color: #94a3b8; font-size: 12px; margin: 0;">
              Du får detta mail för att du har aktiverat notifieringar för nya felärenden.
            </p>
          </div>
        </div>
      </body>
      ...
      try {
        const response = await fetch("https://api.resend.com/emails", {
          ...
          body: JSON.stringify({
            ...
            to: [email],
            subject: `🔧 Nytt felärende: ${payload.title}`,
            html: emailHtml,
          }),
        });
        ...
        if (response.ok) {
          ...
          await supabase.from("notification_logs").insert({
            ...
            reference_table: "fault_cases",
            reference_id: payload.fault_case_id,
    ...
    console.error("Error in send-fault-notification:", error);
