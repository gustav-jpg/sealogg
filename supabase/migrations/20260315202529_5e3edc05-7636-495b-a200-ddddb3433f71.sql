-- 1) Fix mention matching: match against real profile names in same organization
...
BEGIN
  ...
  -- Match @mentions by checking actual profile full_name in same org
  SELECT ARRAY(
  ...
  PERFORM net.http_post(
    ...
    body := jsonb_build_object(
      ...
      'title', '💬 ' || COALESCE(v_commenter_name, 'Någon') || ' nämnde dig i ' || COALESCE(v_fault_title, 'felärende'),
...
-- 2) Ensure triggers are attached (idempotent recreation)
DROP TRIGGER IF EXISTS on_fault_comment_insert_notify ON public.fault_comments;
...
DROP TRIGGER IF EXISTS on_fault_assigned_notify ON public.fault_cases;
