...
export default function EshopInvoices() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ['es_invoices', selectedOrgId],
    enabled: !!selectedOrgId,
    queryFn: async () => {
      const { data, error } = await supabase.from('es_invoices').select('*').eq('organization_id', selectedOrgId!).order('issued_at', { ascending: false });
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? <p className="text-sm text-muted-foreground">Laddar...</p> : rows.length === 0 ? (
              ...
              <Table>
                <TableHeader><TableRow><TableHead>Nr</TableHead><TableHead>Datum</TableHead><TableHead>Förfaller</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Summa</TableHead></TableRow></TableHeader>
                <TableBody>
                  {rows.map((r: any) => (
                    <TableRow key={r.id}>
